...
{
    public class UIElement : MonoBehaviour, UIElementBase
    {
        ...
        private AudioObject showClip => elementData.ShowClip;
        private AudioObject hideClip => elementData.HideClip;
        

        private Vector3AnimationSection MovementSection => elementData.MovementSection;
        [SerializeField, Tooltip("The position the element will move to when it's hiding.")]
        ...
        public void ChangeVisibility(bool v, bool noSounds = false)
        {
            ...
            if (visible != v && !noSounds)
            {
                ...
                if (v)
                {
                    if (showClip)
                    {
                        AudioController.Instance.Play(showClip);
                    }
                }
                else
                {
                    if (hideClip)
                    {
                        AudioController.Instance.Play(hideClip);
                    }
                }
            }
            ...
            if (OpacitySection.UseSection)
            {
                ...
                if (target)
                {
                    ...
                    {
                        OpacitySection.Control(gameObject, val => graphic.Alpha(val), visible, false);
        ...
        {
            elementData = elementData == null ? ScriptableObject.CreateInstance<UIElementData>() : elementData.Clone();
            InitUIData();
        ...
        {
            elementData = numberUIElementData.Clone();
            InitUIData();
        }
