...
namespace PGSauce.Games.BoneGenerator
{
    public class RagdollCreator : MonoBehaviour
    {
        ...
        public BodyPart leftArmOne;
        public BodyPart leftArmTwo;
        public BodyPart rightArmOne;
        public BodyPart rightArmTwo;
        public BodyPart leftLegOne;
        public BodyPart leftLegTwo;
        public BodyPart rightLegOne;
        public BodyPart rightLegTwo;
        ...
        ArrayList bones;
        BoneInfo rootBone;
        private ConcreteSerializableDictionary<BodyPart,BonesGenerator> _bones;
        ...
        public void OnWizardCreate(ConcreteSerializableDictionary<BodyPart, BonesGenerator> bones)
        {
            _bones = bones;
            PrepareBones();
            CalculateAxes();
        ...
        Bounds GetBreastBounds(Transform relativeTo)
        {
            // Pelvis bounds
            Bounds bounds = new Bounds();
            bounds.Encapsulate(relativeTo.InverseTransformPoint(_bones[leftLegOne].Position));
            bounds.Encapsulate(relativeTo.InverseTransformPoint(_bones[rightLegOne].Position));
            bounds.Encapsulate(relativeTo.InverseTransformPoint(_bones[leftArmOne].Position));
            bounds.Encapsulate(relativeTo.InverseTransformPoint(_bones[rightArmOne].Position));
        ...
        {
            float radius = Vector3.Distance(_bones[leftArmOne].Position, _bones[rightArmOne].Position);
            radius /= 4;

            SphereCollider sphere = Undo.AddComponent<SphereCollider>(_bones[headBodyPart].gameObject);
            sphere.radius = radius;
            Vector3 center = Vector3.zero;

            int direction;
            float distance;
            CalculateDirection(_bones[headBodyPart].transform.InverseTransformPoint(_bones[torso].Position), out direction, out distance);
            if (distance > 0)
        ...
        void CalculateAxes()
        {
            if (headBodyPart != null && torso != null)
                up = CalculateDirectionAxis(_bones[torso].transform.InverseTransformPoint(_bones[headBodyPart].Position));
        ...
        void PrepareBones()
        {
            ...
            AddMirroredJoint("Arm", leftArm, rightArm, "Pelvis", worldUp, worldForward, -70, 10, 50, typeof(CapsuleCollider), 0.25F, 1.0F);
            AddMirroredJoint("Elbow", leftElbow, rightElbow, "Arm", worldForward, worldUp, -90, 0, 0, typeof(CapsuleCollider), 0.20F, 1.0F);

            AddJoint("Head", head, "Pelvis", worldRight, worldForward, -40, 25, 25, null, 1, 1.0F);
        }

        private Transform head => _bones[headBodyPart].transform;

        private Transform rightElbow => _bones[rightArmTwo].transform;

        private Transform leftElbow => _bones[leftArmTwo].transform;

        private Transform rightArm => _bones[rightArmOne].transform;

        private Transform leftArm => _bones[leftArmOne].transform;

        private Transform rightKnee => _bones[rightLegTwo].transform;

        private Transform leftKnee => _bones[leftLegTwo].transform;

        private Transform rightHips => _bones[rightLegOne].transform;

        private Transform leftHips => _bones[leftLegOne].transform;

        private Transform pelvis => _bones[torso].transform;
        public Rigidbody TorsoRb => FindBone("Pelvis").anchor.GetComponent<Rigidbody>();
