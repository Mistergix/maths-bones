...
namespace PGSauce.Core.PGEditor
{
    public class FSMViewerEditorWindow : EditorWindow
    {
        ...
        private HashSet<SoStateBase> _visited;
        private HashSet<SoStateBase> _allStates;
        private SoStateBase _initialState;
        private List<AnyTransitionBase> _any;
        ...
        private IMonoStateMachineWithSo _stateMachine;
        ...
        private void RegenerateGraph()
        {
            ...
            _visited = new HashSet<SoStateBase>();
            _allStates = new HashSet<SoStateBase>();
        ...
        private void GenerateAny()
        {
            ...
            var newStates = new HashSet<SoStateBase>();
        ...
        {
            _initialState = null;
            foreach (var o in Selection.GetFiltered(typeof(IMonoStateMachineWithSo), SelectionMode.Assets))
            {
                _stateMachine = (IMonoStateMachineWithSo) o;
                _any = _stateMachine.GetAny();
                _initialState = _stateMachine.InitialState();
                break;
            }
        }
        ...
        private void GenerateEdges()
        {
            foreach (var fromState in _allStates)
            {
                ...
                foreach (var transition in transitions)
                {
                    ...
                    var edge = new DependencyEdge($"{fromState.StateName} => {toState.StateName}",
        ...
        private void FindAllStates(SoStateBase soStateBase)
        {
            if (soStateBase.IsNullState) { return; }
            if (_visited.Contains(soStateBase)) { return; }

            _visited.Add(soStateBase);
            _allStates.Add(soStateBase);

            foreach (var t in soStateBase.GetTransitions().Select(transition => transition.GetTargetState()))
            {
                if (!t.IsNullState)
                {
                    _allStates.Add(t);
